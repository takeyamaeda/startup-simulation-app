...
仕様書に基づき、以下の機能とモダンなビジュアルインターフェースを実装しました。

### 6.1 1画面完結型「真のシングルビュー」ダッシュボード
* ブラウザの縦スクロールを完全に排除し、ビューポート（100vh）に完全に収まる左右分割スプリットビューを採用。
* **左モニター画面**: ヘッダー、データ推移チャート（Rechartsによる面グラフ）、色分けされたタイムライン経営ログ。
* **右操作パネル**: コントロール群を「経営コマンド」「人事・HR」「競合リーダーボード」の3つの切り替えタブに集約し、情報の過密を防ぎながらスムーズな操作を実現。

### 6.2 状態の永続化（LocalStorageセーブ）
* Zustandの `persist` ミドルウェアを実装。ブラウザをリロードしたり閉じたりしても、現在のターン数・資金・従業員・歴史ログ・推移チャートデータが自動的に保存され、ゲームの途中から再開可能。

### 6.3 専門用語のビジュアルインジケータ化
* 一般のユーザーが「TAM」や「ARPU」といったビジネス用語を理解しやすいよう、業界選択画面に「市場規模（TAM）」「顧客単価（ARPU）」を指し示す**紫と青のパラメータプログレスバー**を実装。

### 6.4 高コントラスト選択ハイライト
* クリックされたボタン、現在選択されている業界、難易度、およびプロダクト開発モードには、太い境界線と明るいネオングロー影を付与し、アクティブ状態を視覚的に強調。

### 6.5 将来用Supabase SQLスキーマ
* プロジェクトルートの `supabase/schema.sql` に、ユーザープロフィール、Zustandセーブステート全体を保存する JSONB セーブデータテーブル、クリアした実績を記録するリーダーボードのテーブル設計およびRLS（行レベルセキュリティ）ポリシーを格納。

---

## 7. ローカル開発環境での起動方法

開発サーバーを立ち上げてローカルでゲームを起動できます。
